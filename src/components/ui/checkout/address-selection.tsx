        ...
        {view === 'list' && (
          <div className="space-y-4">
            ...
            {savedAddresses.map((address) => (
              <div key={address.id} className="border rounded-lg p-4 flex justify-between items-center">
                <div className="text-sm">
                  <p className="font-medium">{address.street}</p>
                  <p>{address.city}, {address.state} {address.zipCode}</p>
                </div>
                <Button onClick={() => handleSelectAddress(address.id)} disabled={isPending} size="sm">
        ...
        {view === 'form' && (
          <form action={formAction} className="space-y-4">
             {savedAddresses.length > 0 && (
                <button type="button" onClick={() => setView('list')} className="text-sm text-indigo-600 hover:underline mb-4">
            ...
            <div>
              ...
              <Input id="email" type="email" {...register('email')} readOnly={!!userEmail} className={userEmail ? 'bg-gray-100' : ''}/>
